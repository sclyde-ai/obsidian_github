- 体 field
    - 定義 def
        ...
        - 加法 addiction
            ...
            - 単位元 identity
                $$ \exists 0 \in \mathbb R x+0=0+x=x, \forall x \in \mathbb R$$
            - 逆元 inverse
                $$ \exists -x \in \mathbb R x+(-x)= (-x)+x = 0, \forall x \in \mathbb R$$
        - 乗法 multiplication
            - 結合律 associativity
            ...
            - 単位元 identity
                $$ \exists 1 \in \mathbb R x \cdot 1= 1\cdot x = x, \forall x \in \mathbb R$$
            - 逆元 inverse
                $$ \exists y \in \mathbb R x \cdot x^{-1}= x^{-1} \cdot x = 1, \forall x\in \mathbb R$$
        - 分配律 distributivity
...
- 全順序 total order
    ...
    - 最大値/最小値
        - 最大値
            $$ \max A: \exists a \in A, \forall x\in A, x \leq a $$
        - 最小値
            $$ \min A: \exists a \in A, \forall x\in A, a \leq x $$
        - 最大値/最小値が存在しない集合
    ...
    - 上界/下界
        ...
        - 有界
            $$ U(A) \ne \phi \land L(A) \ne \phi $$
        - 上に有界と最大値
            $$ \exists \max A \in A \Rightarrow \max A \in U(A) $$
        - 下に有界と最小値
            $$ \exists \min A \in A \Rightarrow \min A \in L(A) $$
    - 上限/下限
        - 上限
            - 半順序集合
                $$ \sup A = \min U(A) $$
            - 実数
                $$ \forall x \in A, x \leq \alpha \\ \forall \epsilon \in \R_{++}, \exists x \in A, \alpha - \epsilon \leq x \leq \alpha $$
        - 下限
            - 半順序集合
                $$ \inf A = \max L(A) $$
            - 実数
                $$ \forall x \in A, \alpha \leq x \\ \forall \epsilon \in \R_{++}, \exists x \in A, \alpha \leq x \leq \alpha + \epsilon $$
        - 上限と最大値
            $$ \exists \max A \in A \Rightarrow \max A = \sup A $$
            $$ \exists \sup A\in A \Rightarrow \max A = \sup A $$
        - 下限と最小値
            $$ \exists \min A \in A \Rightarrow \min A = \inf A $$
            $$ \exists \inf A \in A \Rightarrow \min A = \inf A $$
        - 有限集合
            $$ \sup A = \max A \\ \inf A = \min A $$
        - 近似性質
            $$ \forall \epsilon \in \R_{++}, \exists x \in A, \sup A - \epsilon < x < \sup A $$
            $$ \forall \epsilon \in \R_{++}, \exists x \in A, \inf A < x < \inf A + \epsilon $$
        - 包含関係
...
- 連続性 continuity
    - アルキメデスの原理
        $$ \forall a, b \in \mathbb R \exists n \in \mathbb N, an > b $$
        - 自然数の極限
            $$ \lim_{n \rightarrow \infty} n = +\infty $$
            - 証明 proof
                $$ \forall a, b \in \mathbb R \exists n \in \mathbb N, n > \frac{b}{a} $$
    - 連続の公理
        - デデキントの公理 Dedekind’s axiom
            - デデキント切断 dedekind cut
                $$ A \cup B = \mathbb Q \\ A \cap B = \phi \\ A \ne \phi \land B \ne \phi \\ \forall a, b \in \mathbb R a \in A \land b \in B \Rightarrow a < b $$
                となるA|Bをデデキント切断という
            デデキント切断を実数とする
        - W 上に有界な集合は上限を持つ
            $$ U(A) \ne \phi ,\exists \sup A \in \mathbb R$$
        - M 上に有界な単調増加数列は数列の上限に収束する
            ...
            - W → M
                ...
                $$ \alpha = \sup A $$
                書き換えると
                $$ \forall \epsilon \in \R_{++}, \exists N \in \mathbb N \\ \alpha -\epsilon < a_N \\ \alpha - a_N < \epsilon $$
                単調増加なので
                $$ \forall n \geq N, \\ 0 \leq \alpha - a_n < \epsilon $$
                全部書き下すと
                $$ \forall \epsilon \in \R_{++}, \exists N \in \mathbb N, \forall n \geq N, \\ 0 \leq \alpha - a_n < \epsilon \\ \lim_{n \rightarrow \infty} a_n = \alpha
                $$
        - A&K アルキメデスの原理と区間収縮法
            ...
            - 区間収縮法
                ...
                $$ \lim_{n \rightarrow \infty} (b_n - a_n) = 0 \\ \exists c \in \mathbb R \bigcap_{n=1}^\infty I_n = \{c\} $$
            - M → A&K
        ...
        - A&C アルキメデスの原理と任意のコーシー列の収束
            [アルキメデスの原理](https://www.notion.so/216ec42dd04b812087c3d37d46b94f80?pvs=21)
            - コーシー列
                $$ \forall \epsilon \in \R_{++}, \exists N \in \mathbb N, |a_n - a_m| < \epsilon $$
